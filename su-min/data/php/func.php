...
function authorization(){
	...
	if(isset($_POST['enteravtorization'])){
		if(md5($_POST['login']) == '16ca8355099ca0a57e9379d9f38e114c' && md5($_POST['pass']) == 'd38968b4c37c3c08674fbef8147c9037'){
			if(getVar($_POST['rememberMe']) == 'yes'){setcookie("access","16ca8355099ca0a57e9379d9f38e114c",time()+60*60*24*365*10) or die("ошибка куки");}
...
|	   		  -- ВЫВОД HTML ЧАСТЕЙ --				|
|													|
|	Унифицированный ввод head-блока сайта			|
|	(вместе с DOCTYPE) с возможностью включения		|
|	произвольных js и css страниц.					|
|													|
\--------------------------------------------------*/

function echoHead($js = '', $css = '', $noDefIncludes = false){	//параметры без расширений. Могут быть массивы (если несколько) или строки (если один). Если указан третий параметр, то загружаются только переданные параметры. Причем путь указывается полностью
?>
<!DOCTYPE HTML PUBLIC '-//W3C//DTD HTML 4.01//EN' 'http://www.w3.org/TR/html4/strict.dtd'>
<html>
<head>
	<meta http-equiv="Content-type" content="text/html; charset=utf-8">
	<title>site utilit (<?=$_SERVER['SERVER_NAME'];?>)</title>
...
function echoTopRow($pathToIndex = ''){
	//-- очистка сессии --//
	if(isset($_GET['clearSession'])){$_SESSION = array();echo"<script type='text/javascript'>location.href = location.pathname;</script>";}
	echo
	"<div id='topMenuBox'>".
		"<table border=0 width='100%'><tr>".
			...
			"<td>".
				// меню навигации
				"<form method='get' action='".$pathToIndex."index.php' style='display:inline' target='_top'>".
				"<select id='topMenuSelect' name='toplace' onChange='this.parentNode.submit();'>".
					"<option value='index.php'>главная страница".
					"<option value='globals.php'>глобальные массивы и переменные".
				...
				"</select>".
				"<script type='text/javascript'>$('topMenuSelect').value = '".basename($_SERVER['SCRIPT_NAME'])."';</script>".
